...
public class TaskDaoJdbcImpl implements TaskDao {
	...
	public class TaskDtoMapper implements RowMapper<Task> {
		...
		public Task toObject(ResultSet rs) throws SQLException {
			...
			tmp.setCreated(toDate(rs.getDate("created")));
			tmp.setPlanned(toDate(rs.getDate("planned")));
			tmp.setFinished(toDate(rs.getDate("finished")));
			tmp.setCategoryId((Long) rs.getObject("category_id")); // may be																// null
